 ...
 *
 * 两件事分得很开，别混（契约 `ToolResult` 的两件，第 2 轮补锚后**都在契约里**）：
 * - **面向模型的文本**（`ToolResult.output`）＝ 沙箱给的终值，**不因转存而变**；
 * - **记录侧形态**（`ToolResult.content` ／ `tool.result.data.output`）＝ 小则内联、
 *   大则引用；两者**同物**——本文件产出的那一个 `Content` 同时喂给事件与返回值。
 *
