 ...
 * 管理者（`manager.ts` 那一处调用点——三类在那儿按**同一把尺子**判：「这条会话有没有窗口
 * 正看着它」，**正看着就一个字都不说**，见那一处的注）。
 *
 * ⚠️ **三类转换落到屏上的那句话都没有了**（U74「跑完了」/ U79「需要你」/ U86「出错了」，
 * 都是用户定的）——**看着它**那一档「屏上已经有那一行」，印了是复述；**没看着**那一档
 * 该走系统通知 ＋ 未读，而不该落到别人正看的页上（D38）。**一条运行通知都不再往窗口送**，
 * 故 `tui` 的 `noticeReceiptOf` 那一条路**今天不会被叫到**（它作为第二道留着，见 `view.ts`）。
 *
 * ⚠️ **下面那张表说的是「哪几类转换要通知」，不是「哪几类要印一行」**——两件事别混：
 * 三类照旧各留一条通知（**系统通知 ＋ 未读**那两条路要靠它），
 * 只是它们**都不再印到任何一页上**。
 *
 * ## 三条说、三条不说
 *
