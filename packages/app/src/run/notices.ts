 ...
 * 管理者（`manager.ts` 那几处调用点——「跑完了」那一档在那儿按「这条会话有没有窗口正
 * 看着它」判，**正看着就一个字都不说**，见那一处的注）；**那句话怎么说**归外壳
 * （`tui` 的 `noticeReceiptOf`——只有它认得出那条会话的标题）。
 *
 * ⚠️ **「跑完了」那一档的这句话没有了**（U74，2026-09-25 用户定）——`noticeReceiptOf`
 * 对它**不再产出**任何一行。它留在下面那张表里的由头只剩「**不许播报『还在跑』**」那半。
 *
 * ## 三条说、三条不说
 *
 * | 说 | 不说 |
