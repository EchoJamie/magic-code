...
function bindManager(options: ManagerOptions, now: () => number): Manager | undefined {
   ...
   * ## 「在场」的判据按类分（U74 / U79）
   *
   * | 那一类 | 「有窗口正看着它」看的是 | 看着它 | 没看着 |
   * | --- | --- | --- | --- |
   * | `done`（U74） | **这条会话**有没有窗口正看着它 | **一个字都不说** | 系统通知 ＋ 未读 |
   * | `needs-you`（U79） | 同上 | **一个字都不说**（且**不广播**） | 系统通知 ＋ 未读 |
   * | `failed`（U74 起明写「一个字没动」） | 照旧**有没有窗口连着** | **广播**给那几个窗口 | 系统通知 ＋ 未读 |
   *
   ...
   * （U49 的接回快照已承担，**本单不重造它**）· **它不回执、不广播** ·
   * **一个窗口都没有时，才加一记本机系统通知**。
   *
   * - **不回执**：卡**就在你眼前**——「等你定夺」那句话是把同一件事再说一遍。故外壳那一头
   *   `noticeReceiptOf` 对它也不产出（`view.ts`，第二道）；
   * - **不广播**：这一类**永远不往窗口上送**（下面那条 `if (watched)` 的广播只走 `failed`）。
   *   设计那句是「**一件事归它自己那条会话**——回执与裁决都只落在它说的那条会话里，
   *   不落到别的会话的页上」。旧的「有没有窗口连着」之下，A 页开着就会印进 A（D38）。
   *
   * ⚠️ **判据是「这条会话有没有窗口正看着它」，不是「有没有窗口连着」**（两类同此）——
   * 两者差在「A 会话开着、B 会话在等你 / 跑完」这一形：按「有没有窗口」，B 那件事
   * **两头都不说**（系统通知不弹、回执又不该印），那一条就没人告诉用户了。
   * 「看着」由窗口自己的目标报（`watchersOf`）。
   *
   * ⚠️ **`failed` 与停止那一类一个字没动**（U74 与 U79 两单都明写）：判据照旧是
   * 「有没有窗口连着」、话术照旧由外壳拼。**跨页那一半不在本单**——它今天仍会落到
   * 连着的那几扇窗上，那是另一件事。
   */
  function notify(session: string, kind: NoticeKind, fact: string | number, detail?: string): void {
    ...
    // 有人在场吗（见上表：`failed` 照旧看「连着没有」，另两类看「这条会话有没有人正看着」）
    const watched = kind === 'failed' ? clients.size > 0 : watchersOf(session) > 0

    // **看着它 ⇒ 一个字都没有**（`done` 与 `needs-you` 两档）：
    // 前者那条回执不要了、后者卡就在眼前，两档都「没有落到哪」可言。
    // ⚠️ `failed` **不在这一条里**——它照旧广播给连着的那几扇窗。
    if (watched && kind !== 'failed') return
    ...
    /**
     * **说给窗口听**——只剩 `failed` 走这一条（走到这儿 `watched` 为真时它只能是它）：
     * `done` / `needs-you` 两档在上头就返回了，故这两类**一个窗口都收不到**。
     */
    if (watched) {
  ...
  /**
   * **这条会话此刻有没有窗口正看着它**——`done` 与 `needs-you` 两档的判据（见 `notify` 的注）。
   *
