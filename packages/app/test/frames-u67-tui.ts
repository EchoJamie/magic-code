...
if (import.meta.main) {
  ...
  try {
    // —— ① 一问一答（100×30）——那一句「我输入之后与模型回复之间没有空行」的正题 ——
    {
      ...
      //
      // ⚠️ **U74 改**——**原锚**：`['· 「在吗」那一轮跑完了', '', '› 在吗', '', '⏺ 收到，我在。']`
      //    （头一行是「跑完了」那条回执，它按 D38 记的那一形**排在它说的那一轮正文之前**）。
      //    **为何变**：那条回执**整个撤掉了**（2026-09-25 用户定，见设计 · 会话与运行管理
      //    「通知」那一格）——**不是**改落点、**不是**改次序，是**不要了**。所以这一屏上
      //    连着那行空档一起没了（回执与它自带的那行分段是同一笔账）。
      //    **新锚**：`›` ／ 空行 ／ `⏺` ——本张要判的那件事（两块之间恰有一行）一字未变。
      checkLines(shot, '01 一问一答', ['› 在吗', '', '⏺ 收到，我在。'])
    ...
    // —— ② 助手带工具组（100×30）——助手说了话，紧接着一个工具组 ——
    {
      ...
      // ⚠️ **U74 改**（原锚见 ① 那一处）：那条「跑完了」的回执撤掉了，两处各少一行
      //    （连同它自带的那行分段）——`⏺` 与工具组、工具组与下一条 `⏺` 之间**照旧各一行**。
    ...
    // —— ③ **用户之后直接是工具**（100×30）——工单点名的那一形：中间没有 `⏺` ——
    {
      ...
      // ⚠️ **U74 改**（同 ① 那一处）：回执撤掉之后，`›` 之后紧跟的仍是工具组（这一形一字未变）
    ...
    // —— ④ 工具组夹在两条发言之间（100×30）——再补一条用户交代，看交界两头 ——
    {
      ...
      // ⚠️ **U74 改**（同 ① 那一处）：三处回执各少一行。**交界两头一字未变**——
      //    「工具组 → `⏺ 这一处看过了。`」与「`⏺` → 下一条 `›`」各一行，正是本张要判的。
    ...
    // —— ⑤ 连续两组工具（100×30）——两组各自成块、组内各自紧贴 ——
    {
      ...
      // ⚠️ **U74 改**（同 ① 那一处）：三处回执各少一行；两组工具与它们各自身后的发言
      //    之间的那两行分段**照旧**（组与组之间那句发言把两块分开——这一形一字未变）。
    ...
    // —— ⑥ 只有回执（无工具）（100×30）——回执**贴在前一块尾巴上**，前面不留空行 ——
    {
      ...
      checkLines(shot, '06 只有回执', [
        ...
        //
        // ⚠️ **U74 改**：头一行「跑完了」那条撤掉了（不是这一条——`/clear` 的回执是**命令
        //    回执**，与通知那条路两回事，本张正要看它照旧贴着前一块）
        '· 认得的用法：/clear（不带参数）',
      ])
      check(
    ...
    // —— ⑦ 矮窗 40×10——**多出来的空行要进高度账**（U45/U59 那两次的教训） ——
    {
      ...
      // 矮窗上记录区只剩尾巴——判据落在那一截上：工具组之后**一行**，再接那句发言
      // （⚠️ **U74 改**：原先是「回执之后一行」——那条回执撤了，交界本身一字未变）
      const tail = contentOf(shot)
      check(
        tail.includes('') && tail.at(-1) === '⏺ 完了。',
        '07：矮窗上那一道交界仍在（工具组之后一行、再接 `⏺ 完了。`）',
        JSON.stringify(tail),
      )
