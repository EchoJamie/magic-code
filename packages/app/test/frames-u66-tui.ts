 ...
 * | ③ **反面**（没弹卡的工具） | 先答一笔 `a 本工作区总是允许`，再叫第二笔同类 | **一个卡都没有**，而那一行**照旧报耗时**（从发起到此刻——与改动前同形） |
 *
...
/**
 * `write` 是必闸（整文件覆盖要问）⇒ 一定弹卡，且卡上给的是「y 批准 / n 拒绝」。
 *
 ...
 *
 * 造法：先用「a 本工作区总是允许」答掉一笔轻的 `exec`（`echo` 判成只读、不落路径 ⇒ 凝出的
 * 规则是「`exec` × 只读 × 根内」），第二笔同类（`sleep 2.5`——它也在只读那张表里）
 * **就不再问**。
 *
...
async function noCard(mark: string): Promise<void> {
  ...
  try {
    ...
    const card = await session.capture({ label: `${mark}-07-第一笔的卡` })
    keep(card)
    check(
      card.text.includes('a 本工作区总是允许'),
      `【${mark}】前提：第一笔是**轻**的（卡上有「总是允许」这一格）`,
      card.text.includes('总是允许') ? '' : `（卡上没这一格，造不出反面）${card.text.slice(0, 200)}`,
    )
    ...
    check(
      !running.some((line) => line.includes('y 批准')),
      `【${mark}】反面：第二笔**一个卡都没有**（同类已授权，走的是自动放行）`,
      running.filter((line) => line.includes('批准')).join(' / ') || '（一条都没有——对的）',
    )
    check(
      !mid.text.includes('y 批准'),
