 ...
 * | ③ | **反面**：`failed`（U74 刚落下那一档） | **照旧一条**、**逐字**（`· 「…」出错了：这一轮出错了`） |
 *
 * ⚠️ **①那一张是「不广播」的正面**：改之前，B 那件事会**印在 A 这一页上**
 * （判据是「有没有窗口连着」，见缺陷 [[缺陷/D38 通知回执的落点与次序]]）；
 * 改之后 A 那一页干净，而 B 那件事**另有去处**（系统通知 ＋ 未读——那一格走端口记账，
 * 归 `bun test` 的 `run-stop.test.ts`，本文件一个字不弹：真弹是 `osascript`）。
 *
 * ⚠️ **②那一张走的是既有那一手**（U49 的接回快照，`applyResume` 里挂卡那一段）——
 * 本单**只动回执与广播**，没有重造它；这一张是**它没被碰坏**的物证。
 *
 * ⚠️ **③那一张只判「逐字未变」**：`failed` 照旧按 U74 的口径走（判据仍是「有没有窗口
 * 连着」、话术一字未动）。它**不是**在说「那一档的落点已经理想」——U74/U79 都明写这一档
 * 未动，跨页那一半属另一件事（`失败` 那一行仍会落到连着的那几扇窗上）。
 *
 * ## 跑法
 *
 * ```
...
if (import.meta.main) {
  ...
  try {
    ...
    // —— ③ **反面**：`failed` 那一档**逐字未变**（U74 刚落下，别碰坏） ——
    await typeLine(mine, '再问一句')
    await mine.key('enter')
    await mine.wait({ text: '· 「甲窗那句」出错了：这一轮出错了' }, { timeoutMs: 30_000 })

    const others = await mine.capture({ label: '03-反面-failed照旧' })
    keep(others)
    check(
      has(others, '· 「甲窗那句」出错了：这一轮出错了'),
      '③ `failed` 那一类**照旧一条、逐字**（U74 的口径一处未动）',
      others.text,
    )
