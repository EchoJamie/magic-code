 ...
 * 2. **正文与回车分两次写**——「与手打同形」：人敲完一句再按回车，本来就是两下。
 *    ⚠️ **这条不再是绕开缺陷**（U56 修 D35 之前它是）：那时挤在同一次写里，回车会丢
 *    （`/exit\r` 整块被 Ink 当一串正文），故 `send()` 与 `key('enter')` 非得分两写不可。
 *    现在**挤一块也算数**了——正面那一形由 `scenarios.ts` 的 `packed-enter` 专门钉着。
 *    这里仍按两次写：它照旧是常态动作，判据不该靠「挤块也行了」才过。
...
const KEYS: Record<string, string> = {
  ...
  'ctrl+c': '\u0003',
  /**
   * **展开工具行那些正文**（U72）——外壳那一侧认 `ctrl+o`（`components/app.ts` 的 `toShellKeys`），
   * 而 0x0F 正好落在 Ink「ctrl+字母」那一支（`s <= 0x1a` ⇒ name `o` ＋ ctrl）。
   *
   * 加它是因为 `web_fetch` 的回执正文（收据两行 ＋ 答案）**展开才看得见**，判据要看那几行——
   * 此前只能往 `send()` 里塞一个裸控制码，而那个字节写进用例谁也读不出来。
   */
  'ctrl+o': '\u000f',
...
export type UiAnchors = {
   ...
   *
   * 由头：产品那边那道门**只开一小会儿**（U68 起有它，U74 缩到 1 秒），过了就「再按一次」成了
