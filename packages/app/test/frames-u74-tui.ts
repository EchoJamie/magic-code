 ...
 * | ③ | 出错 ＋ 命令回执 | **别的回执照旧**（`· 「…」出错了：…` · `· 认得的用法：…`），而**那一条**仍然一个字都没有 |
 *
...
if (import.meta.main) {
  ...
  try {
    ...
    // —— ③ **别的回执照旧**（出错那一类 ＋ 命令那一路），而「跑完了」仍然一个字都没有 ——
    await typeLine(session, '出错那句')
    await session.key('enter')
    // ⚠️ 等的是**那一条回执本身**，不是「回到空闲」——出错那一场收在 `▲ 出错` 那一格上
    //    （不是空闲），拿空闲当条件会当场白等到超时（实测栽过一次）
    await session.wait({ text: '· 「在吗」出错了：这一轮出错了' }, { timeoutMs: 30_000 })
    ...
    keep(others)
    check(
      countOn(others.lines, '· 「在吗」出错了：这一轮出错了') === 1,
      '③ 「出错了」那一类**照旧一条**（与撤掉的那一条同一个通知通道）',
      others.text,
    )
    check(
