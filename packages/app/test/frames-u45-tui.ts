 ...
 *
 * 一帧里**两条**满宽 `─`（记录区／交互区之间那条 ＋ **输入区／状态行之间**那条），
 * **同宽同色**；**只有输入行**被夹在当中，状态行在下线**之下**（「一眼分出三块」）。
 * **不许再多线**——这一条就是工单里那四条硬约束的第三条，量的是**整份缓冲**里的条数。
 *
 * ⚠️ **U45 那版把下沿那条加在状态行之下了**（那会儿两条线之间夹的是「输入行 ＋ 状态行」）
 * ——用户 2026-09-25 指出加错了位置：原话是「输入区 **与** 底部状态栏」，划的是那两块。
 * 下面 `checkFrame` 里那两条 `status` 判据随之翻面。
 *
 * ## 跑法
 *
 * ```
 ...
 * **输入行** · **状态行**；**输入行落在两条线之间**、**状态行落在下线之下**（「一眼分出三块」，
 * U59 的位置）。
 */
function checkFrame(shot: Capture, where: string): void {
  ...
  // ⚠️ **状态行在下线之下**（U59）——U45 那会儿它在两条线之间，那是把「输入区 ＋ 状态行」
  //    框起来；线挪到这两块**之间**之后，它就落到线下了。
  check(status > last, `${where}：状态行在**下线之下**（下线之下不再有线）`)
  check(
