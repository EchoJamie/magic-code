...
describe('U42 · MAGIC_HOME 下的真 CLI', () => {
  ...
  test('新基址下**读不懂不回退**——报错点的是新树那条路，不是老树', async () => {
    const room = makeRoom()
    try {
      plantTree(room.home, { provider: 'old', model: 'OLD-MODEL' })
      // 新树这一份**读不懂**（占着位、内容是坏的）。
      //
      // **原锚**（U42）：「新树里哪一份都没有 ⇒ 报错，`path` 指新树」。
      // **为何变**（U41）：**文件不在不再是错**——首次运行就是这样，空配置要能进入接入流程
      //   （U41 的判据：`providers: {}` 照常起步）。于是「报错看落点」这一手改用
      //   **读不懂的内容**来触发：坏内容必须点名报错，这条一个字没松。
      // **新锚**：坏内容 ⇒ 仍 exit 1，报的仍是**新树那条路**。
      mkdirSync(join(room.base, '.magic'), { recursive: true })
      writeFileSync(join(room.base, '.magic', 'config.json'), '{ 这不是 JSON }', 'utf8')
