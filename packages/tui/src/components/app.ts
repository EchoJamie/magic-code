...
export function toShellKeys(
  ...
  //
  // ⚠️ **落到 `other` 那支就不管用了**——那是「没人接的键」的收容处（见下面那一支），
  //    故这一对必须在它**之前**认下来（早先正是漏在这儿：屏上按下去什么都不发生）。
  if (key.ctrl === true && input === 'a') return [{ kind: 'lineStart' }]
  if (key.ctrl === true && input === 'e') return [{ kind: 'lineEnd' }]
