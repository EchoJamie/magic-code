 ...
 *
 * ## ⚠️ 这一支今天**一条都收不到**（U86 起；下面那张表**整张都不上屏**）
 *
 * 「出错了」是最后一格（U74 → U79 → U86 逐类落地）：三类**同一把尺子**——**有人正看着
 * 这条会话 ⇒ 一个字都不说**（那一行本来就在屏上）；**没看着 ⇒ 系统通知 ＋ 未读**。
 * 两档都不走窗口，故 `manager.ts` 的 `notify` **不再往窗口送任何一条运行通知**，
 * 这一支（连同 `ShellOptions.notices` 那条通道）**今天一次都不会被叫到**。
 *
 * **它是第二道**（U74 / U79 落的姿势：即便那一条来了，也印不出来）——⚠️ 但**留着的这
 * 三格已经与设计不符**：设计说「你正看着 ⇒ 不印（屏上已经有那一行）」，而 `failed`
 * 那一格仍会拼出一行字。**今天不可达，故没有可感的后果**；**清掉整条通道**（含这一支
 * 与它的用例）是**另一笔活**，不属于任何一格判据，本单不顺手做。
 */
