...
describe('返工三 · 活动帧不撑满终端（流式增长 · 单条超预算）', () => {
  ...
  /**
   * **空闲按过一次 Ctrl+C**（U46）——输入行上方多出「再按一次 ctrl+c 退出」那一行。
   *
   * 进这条矩阵的理由：那一行**借的是交互区的一格**，矮窗上它多算或**少算**一行，账与屏
   * 就分家、帧顶满终端（U31 那一族的老病）。工单 U46 点名要看的就是这一格：
   * 「会不会把输入行挤掉或撑高 dock」。
   *
   * ⚠️ **这一格得让活动区吃满预算才咬得动人**：账多算一行只在「活区正好卡在预算上」时
   * 把帧顶到终端高度（空活区那一档，多一行少一行都够不着）。故这里另配三条工具记录
   * ——短草稿 ⇒ 交互区只占两行，剩下的都归活区（实测：把高度账里那一项摘掉，这一格当场红）。
   */
  function armed(): Stage {
    const stage = createStage()
    // 流式正文**多过活动区的预算**（40×10 上只有三四行）——「活区吃满」正是这一格要的形状
    stage.feed([
      event('model.delta', {
        channel: 'text',
        text: Array.from({ length: 12 }, (_, index) => `流式 ${String(index + 1).padStart(2, '0')}`).join('\n'),
      }),
    ])
    stage.press({ kind: 'ctrl+c' })
