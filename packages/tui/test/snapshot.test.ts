 ...
 * （说的是 ③ 那会儿的实况：后来 U31 把 `useCursor()` 接上了、那一格也删了，见 ⑥。）
 *
 ...
 *
 * **⑥ 输入行画出来的那一格光标删了（U31）** —— **原锚**：`› ` → **反色空格** → 占位
 *   （`› ` 与占位之间多一格）。**为何变**：那一格与**终端自己的光标**是两个光标——
 *   本单元接了 `useCursor()`（真光标跟着插入点走，见 `composer.ts` 头注），
 *   画的那个就该退场（工单：旧反色格造成双重光标可在输入组件内删去）。
 *   **新锚**：`› ` 紧跟占位（每张快照都少那一格空格）。**动的是**空草稿那一行
 *   （占位 / 等你的答复 / 工作中 / 退避中四种面孔各一张，共 13 张）——**其余逐字未动**。
 *   ⚠️ 真光标**不在这一层量**：`renderToString` 没有终端，读不到光标坐标；
 *   光标归 `spec.dock.test.ts`「输入行 · 光标落在哪」那条真终端的路。
 *
 * **⑦ 空态那句引导语整句删了（U31 三轮）** —— **原锚**：开机那一屏上
 *   「会话在你按下第一次回车时才建立。」那一行（就是 ② ⑤ 两条一路删下来的那一句）。
 *   **为何变**：用户 2026-09-20 定——这句**没有动作价值**、原型早已删掉；真 TUI 里那个
 *   `EmptyState` 随之整块去掉，它的高度项也从活动区预算里去掉（不再为它单算一行）。
 ...
 *
 * **⑨ 交互区下沿多了一条分隔线（U45）** —— **原锚**：全屏只有**一条**分隔线（记录区与交互区
 *   之间），状态行是这一帧的最后一行。**为何变**：用户 2026-09-24 定——「输入行 ＋ 状态行」
 *   底下再封一条，三块（记录区 / 输入区 / 状态区）一眼分得开；两条线**同形制**
 *   （同宽、同色，同一处 `separatorOf`）。
 *   **新锚**：每一张的**末行多一条满宽 `─`**（46 列那张是 46 个），其余**逐字未动**——
 *   这十六张动的是同一处、也只动这一行。
 *   （高度的账跟着进 `CHROME_LINES`，见 `components/app.ts`：不加那一笔，矮窗上帧正好顶满
 *   ⇒ 真光标高一行，那是 U31 那一族的老病。）
 *
 * **⑩ 那条下沿线挪了位置（U59）** —— ⑨ 把它加在**状态行之下**，那是把「输入行 ＋ 状态行」
 *   框起来（装帧），不是划界。**为何变**：用户 2026-09-25 指出「U45 加错了位置」——
 *   原话是「输入区 **与** 底部状态栏」，这条线划的该是**这两块**：它落在**输入行与状态行之间**。
 *   **新锚**：每张的**末两行对调**——原先是「状态行 → 线」，现在是「线 → 状态行」。
...
describe('场景 1 · 启动（空态）', () => {
  test('**新会话，不接续**——空手打开没有会话，状态行报「新会话」', () => {
    ...
    // ⚠️ **换过锚**（U31 三轮）：原锚是那两句空态引导语（`会话在你按下第一次回车时才建立` ＋
    //    它的前半截）——那句 2026-09-20 由用户定删（没有动作价值，原型早已删掉，见 `app.ts`
    //    的注）。这一条的主句是**空手打开报「新会话」**（状态行那三格），一个字没动；
    //    开机那一屏其余长什么样由下面那份快照看住（它也随这句的删除换了一份）。
...
describe('场景 10 · `/model`（同一处、同一开合）', () => {
  test('列表报条目与模型名（**全量**）；开选择器本身**不进记录区**', () => {
    ...
    // ⚠️ 2026-09-23（U41 返修）再改一次：列表末尾常驻了三条**入口行**（连接供应商 /
    //    管理连接 / 刷新模型），故这一屏比上一版多三行——快照随新形态更新。
