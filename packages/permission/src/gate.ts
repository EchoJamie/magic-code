...
export function createPermissionGate(options: PermissionGateOptions): PermissionGate {
  ...
  return {
    decide(call, ctx, callRef) {
      ...
      //
      // ⚠️ **两条来路各有一个例外，集成时别互相盖掉**（两个单各加了半个条件）：
      // - **命中的规则**：判**轻**时放行；**外发那一件按域名**（U72，见 `byHost`）——那是
      //   规则作者明确点过的域名，与"轻重"无关，故它与 `weight` 是**或**的关系；
      // - **全放行**（U73）：**只放判轻的**——它替的是"每件都问"，不是"必闸也不问"。
      //   （⚠️ **这一条正要改**，见工单：用户已定「**连必闸也放**」。）
      if (
        (hit !== undefined && (weight === 'light' || byHost(hit, face))) ||
        (allowAll && weight === 'light')
      ) {
