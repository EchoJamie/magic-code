...
const SUMMARY_INSTRUCTION = [
  ...
  '把它压成一份交接摘要，供接下来接着干活的你使用。必须含四件：',
  '① 任务与状态：用户在做什么、进行到哪一步；',
  '② 关键决定：已经定下的做法与理由，含被否决的路线；',
  '③ 未完成事项：还没做完、待确认、待用户答复的；',
  '④ 触碰文件面：动过或读过的文件 / 目录 / 命令，按路径列出。',
  '要求：直接给摘要正文；保留具体路径、命令、标识符与数字，不要写成泛泛的概述；',
  '不要编造记录里没有的事——不确定的写「未确认」；不要复述这份指示。',
  // 篇幅这一句不是客套：摘要与它顶掉的旧段**等长**时，压缩就白做了（真跑实测：
  // 不写这一句，模型会照着四要素铺开成长文，压完的上下文比压前还长）
  '篇幅宁短勿长——通常几百字；旧段很长时可放宽，但别写成原文的复述。',
].join('\n')
