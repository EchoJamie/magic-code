 ...
 *   文件与目录那两处写的就是**路径**（那是这份材料本身）；工作区外那份另外标一句
 *   「只读附件」。
 *
 * ⚠️ **技能那一处不再报来源**（2026-09-25 用户裁）：原抬头第三件是 `label`，由头是
 * 「同名两份靠它分得开」；同名**只留一条**之后（判定在发现层，见 [[设计/技能]]）
 * **名字已经唯一**，来源对模型是没有信息量的额外字。给**记录与体检**的那两处留着
 * （会话条目载荷、`magic --check`）——那是依据，不是给模型看的材料。
 *
