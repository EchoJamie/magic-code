 ...
 *
 * ⚠️ **不报「太多」**（2026-09-20 四轮裁）：停批的触发有三种（目录读不动 · 层级太深 ·
 * 份数或总量到顶），**后两种与「多」无关**——旧句「规约太多，一次装不下」放在不可读那一屏上
 * 就是一句失准的话。这一格只报**没读完整**这个事实，**是哪一种由来源诊断说**（见 `overflowText`）。
 */
 ...
 * 这正是工单说的「不静默执行或无限重试」。
 *
 * **原因不在这儿编**（2026-09-20 四轮裁）：触发有三种（目录读不动 · 层级太深 · 份数或总量
 * 到顶），**是哪种、在哪一处，由来源诊断给**（`problems`，用户跑 `magic --check` 看得到）。
 * 旧正文把三种都写成「多到装不下」——不可读与层级太深那两屏上，那是一句失准的话；
 * 而**为此各造一套回执**又是多出来的第二套说法。故只报「没读完」，把原因指回诊断那一处。
 */
export function overflowText(): string {
  return (
    `${UNEXECUTED_OVERFLOW}\n` +
    `这个项目里的规约这一趟没能完整读完（哪一处、为什么，见 magic --check 那份诊断——` +
    `目录没看成 · 层级太深 · 份数或总量到顶，都可能）。` +
