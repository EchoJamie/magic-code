 ...
 * **U89 给它补了一面读**（`running()`）：发起与停之外，还要答得出「**此刻哪些还在跑**」
 * ——「还在跑」要进模型那个每回合重建的块，而这件事只有登记自己知道（见那一面的注）。
 *
...
/**
 * **一条还在跑的后台运行**（`BackgroundRuns.running()` · U89）——读面上的一行。
 *
 * 三件：**认得出它**（id ＋ 交出去的那条命令）· **输出在哪儿**（那个文件）。
 * 「还在跑」这件事由**在不在这个列表里**说，不在行里再带一个布尔——列出来就是还在跑。
 *
 * ⚠️ **它不是 `BackgroundFinish`**：那一位说的是「它**后来了结**成什么样」（跑完那一声），
 * 这一位说的是「它**此刻还站着**」——两件事，一个说结局、一个说现状。
 */
export type BackgroundRunning = {
  /** 发起时回执上给过的那个 id（`bg-N`）。 */
  readonly id: string
  /** 交出去的那条命令原文（照原样，不加工——加工在显示那一侧）。 */
  readonly command: string
  /** 它的输出文件（工作区之外）——模型按需 `read` 的就是这一份。 */
  readonly outputPath: string
}
...
export interface BackgroundRuns {
  ...
  /**
   * **还在跑的那些**（按交出去的次序）——**现读**：读一次就是一次当下的读数。
   *
   * ## 为什么要这一面（U89）
   *
   * 「它还在跑」这件事此前**问不到**：登记只在自己内部那本 `live` 账上，而模型需要的
   * 正是这一件——**上下文里没有它，模型会重复启动同一条命令**（设计 · 提示词与指令 甲 ②
   * 点名的那个后果）。故读面立在这儿：**一处产出，模型那一侧每请求现取一次**。
   *
   * ## 只列**真没退出的**
   *
   * 跑完了 / 被停掉的**不在列**——那是 `onFinish` 那一声的事（它说的是「结束」，
   * 这一条说的是「此刻还在」）。两者的判据同源（同一本 `live` 账），故不会各说一套：
   * `settle` 摘账的那一刻起，它就不在这个列表里了。
   */
