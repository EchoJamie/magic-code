 ...
 *
 * **来源与范围分开报**（技术方案 · 记录：「保留来源 / 作用域 / 内容版本」）——
 * `kind` 说来自哪一类入口，`root` / `scope` 说管到哪儿，`version` 说这是哪一版。
 */
export type ProjectRule = {
  ...
  /**
   * **内容版本**——**这份文档**是哪一版：真路径 ＋ **实际范围**（根 · 作用目录）＋
   * 生效模式 ＋ 正文的摘要。
   *
   * 判据是它：「相同版本不循环拦截」（已送达的那一版不再拦第二次），改过的版本另算一版。
   * ⚠️ **身份也在里面**（不只是内容）：两份正文一模一样的规约（同一套约定按目录铺开）
   * 是**两份**——不含路径的话它们共用一个版本号，送过一份就等于「两份都送过」，
   * 另一份的那条约定**永远不送**。判「改没改」看内容，判「送没送」看身份。
   *
   * ⚠️ **范围也属于身份**（2026-09-20 裁）：同**一份物理文件**经两处进来、管的是两摊
   * （根那份 `AGENTS.md` 与 `src/AGENTS.md` 软链到同一个文件）时，范围不同就是**两条**。
   * 版本里不含范围的话，送过其中一条就等于「两条都送过」——scope 小的那条永远不送。
   */
...
export type RulesLoad = {
  ...
  /**
   * **这一趟有没有因为上限而丢掉材料**——`true` ＝「回来的是全的」这句话**不成立**。
   *
   * 由头（2026-09-20 裁）：份数 / 总量到顶时加载器**停下并报出来**（这一条本就有），但
   * **消费方光看 `documents` 是看不出来的**——被上限挡在外面的那些**压根不在列表里**，
   * 于是「目标上的规约都送到了吗」这个问题会被答成「送到了」。对话域据本字段把那种情形
   * **当成未送达**处理：宁可停下来明说，也不在规约没齐的情况下动手（不静默执行）。
   *
