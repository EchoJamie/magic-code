...
export type ModelCapabilities = {
  ...
  /**
   * **接受图片输入**（U37）——`false` ＝**明确不支持**，`true` ＝明确支持，缺省＝**未知**。
   *
   * 三态与 `chat` 同规（未知 **不冒充**「不支持」）：本位的用处只有一个但很硬——
   * **带图的那一条交代在发出去之前**就能拦下来（设计 · 文件与图片：「模型明确不支持图像时
   * 保留输入，提示换模型或移除图片，**不能静默丢图发文字**」）。
   *
   * ⚠️ **未知照发**：不知道就如实按「不知道」办——请求真失败就**如实报错**
   * （「未知能力的实际请求失败如实报错」，设计原文），保存的附件仍可取回。
   * 反过来把未知当「不支持」拦下来，等于凭空禁掉一批其实能看图的模型。
   */
...
export type ProviderModelOverride = {
   ...
   *
   * 今天它主要承接一位：`image`（这家这个型号到底吃不吃图）。由头是**实测打不通的那条路**：
   * 供应商的列表接口多数只给 `id`（MiniMax / DeepSeek 都是），「明确不支持图像」因此
   * **没有**API 依据可循——而设计又禁止按型号名猜。剩下的正当来路只有一条：
   * **用户自己说**（他手里有那家的文档与实测）。故给这一位，与 `limits` / `traits`
   * 同一优先级（最高档：「用户对该连接与精确模型的明确覆盖」）。
   *
   * ⚠️ **它不是「猜的兜底」**：不写这一位就是**未知**——未知照发、失败如实报
   * （见 `ModelCapabilities.image`）。
   */
  readonly capabilities?: ModelCapabilities
}
