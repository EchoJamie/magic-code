...
export type EventKind =
  ...
  | 'model.switched'
  // skill——**一次技能材料真的进了上下文**（U33）；**不落库**（依据在条目载荷里）
  | 'skill.used'
  // 控制 · 输入——**一次提交的收场**（收下 / 没跑，U33）；**不落库**
  | 'input.settled'
...
export type EventDataOf = {
  ...
  //
  // **产出时机＝主文确实进入本次上下文之后**（不是选定那一刻、也不是读取成功那一刻）：
  // 前半程（选定只绑草稿）与失败路径都没有这一条。用户条目的材料在装配时随上下文送出，
  // 故产出点就在那一趟装配之后——「已使用」于是是一句**当时为真**的话。
  //
  // **不落库**：依据在**条目载荷**里（`UserPayload.skills` 的名字 / 来源 / 版本 / 正文）
  // ——重放读的是那份，落库＝把同一件事存第二遍。这条事件的读者是**当下的屏**。
  'skill.used': {
    /** 这一次实际送达的技能。空数组＝不可能出现（产出方只在送到之后发）。 */
    readonly skills: readonly UsedSkill[]
  }
  // 控制 · 输入——**一次提交的收场**（U33）。两格：收下了 / 没跑。
  //
  ...
  //
  // **出的时机**：给了 `ref` 的提交一律回一条（收下了／没跑）；**没给 `ref` 但没跑成**的
  // 也回一条（失败不静默）——纯文本、平淡收场的那些**一条都不发**（旧路径一字不动，
  // 也不给观测添噪声：正常轮次的起止本来就是 `turn.*` 在说）。
  'input.settled': {
    /** 配对键——外壳在 `UserInput.ref` 上给的那一个；没给就不在此带键（不编）。 */
    readonly ref?: string
    /** 收下了没有。`false` ＝ 这一条**没跑**（技能取不到等），缘由在 `reason`。 */
    readonly ok: boolean
    /**
     * 没跑成的缘由（**说给人听的一句话**，含是哪一份来源出的问题）。
     *
     * `ok: true` 时不给——成事不必解释。
     */
    readonly reason?: string
...
export const TRANSIENT_EVENT_KINDS: readonly EventKind[] = [
  ...
  // （`UserPayload.skills`：名字 · 来源 · 版本 · 正文），落库＝把同一件事存第二遍。
