...
export type EventKind =
  ...
  | 'paths.catalog'
  // 控制 · 路径——**选定那一条认出来了**（U62）：`paths.identify` 的答复；**不落库**
  | 'paths.identified'
...
export type EventDataOf = {
  ...
  'tool.decision.request': {
    ...
    readonly external?: boolean
    /**
     * **这一次发往哪个域名**（U72）——只有取网页那一件会带，且只在地址里认得出域名时。
     *
     * 两位用处，都在**这一张卡**上：
     * - **说清去向**（设计 · 网页与搜索：「卡上要说清发给哪个域名」）——外发这件事的
     *   判断材料就是「发给谁」，光有工具名答不出它；
     * - **「总是允许」按域名记**（同处）——那一格的授权**落在域名上，不落在工具上**，
     *   故卡上得把「按哪个域名」写出来；`a` 在必闸件上**只有带这一位时才给**
     *   （见 `@magic/tui` 的 `decision.ts` / 答复键那一处）。
     *
     * 缺席＝这一次没有域名这一维（既有那些工具一个字节都不变）。
     */
  ...
  /**
   * **交出去的那条命令跑完了**（U70）——给**屏**的那一声，不是给模型那一句。
   *
   * 两件事分开（设计 · 工具执行与权限 · `exec` 的后台那一形）：
   * - **给模型**的那一条走**交代通道**（一条 `user` 条目，载荷带 `notice`）——它要进上下文，
   *   让模型自己决定读不读那个文件；
   * - **给屏**的就是这一条：`· 后台命令结束了…` 那一行回执。它**不落库**——「当时屏上
   *   闪了一句什么」不是重放要的东西，那件事另有痕（上面那条条目）。
   */
  'exec.background.done': {
    // 不落库——实时订阅专用
    /** 那条运行的 id（回执上点名用它；停也按它停）。 */
    readonly id: string
    /** 交出去的那条命令（回执上照原样给一眼，好认出是哪一件）。 */
    readonly command: string
    /** **它的输出文件**（工作区之外）——模型要读的就是这一份。 */
    readonly outputPath: string
    /** 命令自己跑到头且 exit 0 才算成；非 0 / 被信号收掉都不算。 */
    readonly ok: boolean
    /** 退出码；读不到（被信号收掉一类）时为 `null`——**不编一个数出来**。 */
    readonly exit: number | null
    /**
     * **是「停掉」那一支**（用户按 id 停的），不是自己跑完的。
     *
     * 分开说，是因为「跑完了」与「被我停掉了」对用户不是同一件事（设计 · 会话与运行管理 ·
     * 停止那一节：「停了什么、停到哪一步」要说得出来）。缺省＝自己结束的。
     */
  ...
  'input.settled': {
    ...
    readonly reason?: string
    /**
     * **这一份草稿还回输入行吗**（U60）——缺省还（U33 起的老规矩：没送出就不丢稿）。
     *
     * `false` 只有一个由头：**下一步要敲的是一条命令**（还没接供应商时得先去
     * `/model connect`）——稿子还回输入行的话，那条命令会接在稿子尾巴上
     * （`你好/model connect`），成了一句谁也不认的话。
     *
     * ⚠️ **不是「把稿子丢掉」**：外壳那边它仍在 `↑` 历史里（交出去那一刻就记下了），
     * 故这儿的取舍是「换个地方放着」；说清它在哪儿是**产生方**的事（那一句
     * `reason` 自己带着「原稿在 ↑ 里」）。
     */
  ...
  // 控制 · 输入——**本次交代里引用了、却没被读过的材料**（U63）。**不落库**。
  //
  // **由头**（设计 · 终端交互 · 2026-09-25 补的那半条）：文件 / 目录 / 技能的送达方式改成
  // 「模型按需自读」之后，**「引用了」不等于「看过了」**——只配「读了要说」那一半不够：
  // 模型没读时那一轮结束**屏上什么也没有**，用户照样以为它看了。故收束时把话说出来。
  //
  // **出的时机＝一次交代收束时**（不是每一轮：多轮工作里模型可能在后面才读，
  // 逐轮报会把同一件事说好几遍）。这一条**只报名字**（材料在交代里怎么写的就怎么报，
  // 见 `refs[].marker`）——用户据它认得出是哪儿处引用。
  //
  // **不落库**：它是**读出来的**（引用了哪几份在 `UserPayload.refs` 里，读没读在工具条目里）
  // ——照 `skill.used` 的先例：重放要的是「当时用了哪一份材料」，不是「屏上闪过一句什么」。
  'input.unread': {
    /**
     * 这一轮**没读到**的材料——按它们在交代里的次序（`at`）。
     *
     * **空数组不发**（产出方只在真有的时候发）：收束时报一句「一份都没落下」是噪声。
     */
  ...
  /**
   * **选定那一条认出来了**——`paths.identify` 的答复（U62 · 图片的名字）。
   *
   * 有意答得**只有这一件**：它是不是一张图（不是 ⇒ 什么都不用说，照 `@路径` 那个块走）。
   * 是的话给**内容身份**与那几格人读的东西——外壳据此把那一处写成 `Image#N`。
   *
   * **不落库**：与 `paths.catalog` 同一条——它是**读出来的**（内容就在盘上），且这一问
   * 只是「这一条是什么」的当场答复；那一处引用到底带了什么材料**另有痕**
   * （`user` 条目的载荷 `refs`：位置 · 来源 · 实际交付内容），重放读的是那份。
   */
  'paths.identified': {
    /** 认的是哪一条——**原样回声**（外壳据它认领自己那一处引用；问几次都不混）。 */
    readonly path: string
    /**
     * **它是一张图**——认得出来才有这一格；**没有＝不是图**（或不在了 / 读不了）。
     *
     * ⚠️ **不给「为什么不是图」**：那不是这一问要答的事。引用本身照旧成立
     * （`@路径` 那个块一个字都不变），真到提交那一刻取不到，由**那一条**如实报错
     * ——「选入的时候就先报一串」会把一件还没发生的事说在前头。
     */
    readonly image?: {
      /** MIME（按字节认出来的那一种）。 */
      readonly mime: string
      /** 文件名（人读）——**元数据，不是身份**（图不一定来自文件）。 */
      readonly name: string
      /** 字节数——给「这一条一共多大」一个读数（**不含字节本体**）。 */
      readonly bytes: number
      /** **内容身份**：字节的 sha256（`records/blobs.ts` 那一刀），也就是 `BlobRef`。 */
      readonly blob: string
      /** 来源的人读写法（相对默认根 / 绝对）——记录与抬头要它。 */
      readonly label: string
    }
  }
...
export const TRANSIENT_EVENT_KINDS: readonly EventKind[] = [
  ...
  'model.retry',
  'tool.output.delta',
  // 后台命令结束的回执同列的理由（U70）：它是**一次实时通报**——同一件事的**条目**已经
  // 落进会话了（那条 `user` 条目，载荷带 `notice`），落库＝把同一件事存第二遍。
  // 重放要的是「当时跑过哪些后台命令、结果如何」（读条目就有），不是「屏上闪了一句什么」。
  'exec.background.done',
  ...
  'paths.catalog',
  // 选定那一条的认出结果同列的理由（U62）：与 `paths.catalog` 同一条——它是**读出来的**
  // （内容就在盘上），且这一问只是「这一条是什么」的当场答复（问几次都不混）。
  // 那一处引用到底带了什么材料同样**另有痕**（`user` 条目的载荷 `refs`：位置 · 来源 ·
  // 实际交付内容，含图片那一份 blob），重放读的是那份。
  'paths.identified',
