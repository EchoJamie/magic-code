...
export type RunNotice = {
   ...
   * 再汇总一遍是重复念。
   *
   * ⚠️ **「在场」的判据三类同此**（U74 → U79 → U86 逐类落地）：**「这条会话有没有窗口
   * 正看着它」**，**不是「有没有窗口连着」**——三类的回执都撤了，「跟着」它们的那一格
   * 也就不能按「有没有窗口连着」算（A 页开着、B 会话跑完 / 在等你 / 出错：
   * B 那件事正要说给用户听）。
   *
   * ⇒ 走到「留一条通知」这一步时，**必然是没人看着它**（看着就不留了），
   * 故今天落盘的每一条都是 `unread: true`——这一位留着是**给读盘那一头**的形制。
   */
