...
/**
 * **一份当前计划笔记**（U34 · **U90 加 `goal`**）——目标 ＋ 有序步骤清单 ＋ 可选的
 * 短文本辅助笔记。
 *
 * 用户可见的那份清单就是它：**`goal` 顶格当表头**（这件事是什么），`steps` 退一级挂在
 * 它下面；「约束 · 已确认事实 · 未解决问题 · 回查线索」仍在 `notes`（一段自由文本，
 * 可为空串）。**不填空表**：没有要记的就留空，不为形式产出一堆字段。
 *
 * ⚠️ **`goal` 是它自己的一格，不是从别处猜来的**（U90）：它**不是** `notes` 的第一行、
 * 也不是从步骤文字里推的——用户看得见的那一行必须由模型明写。故这一格**可选**：
 * 没给＝**整个键不在场**（不是空串），清单上那一行**不出现**，不留空表头。
 *
...
export type PlanNote = {
  /**
   * **这件事要得到的结果**——一句短文本，用户会在清单顶上看到它（U90）。
   *
   * 可选：**不给＝没有目标**，清单上那一行不出现（不留空表头）。
   *
   * ⚠️ **空白在写面就收掉了**（`plan_update` 的参数解析：`goal` 空白 ⇒ 与没给同义，
   * **整个键不写进来**）——故读到的那一份里**在场就是有内容**，读侧判「键在不在场」即可，
   * 不必各处再判一遍它是不是空的（判据两处写，迟早两处对不上）。
   */
...
export type UserPayload = {
  ...
  /**
   * **这一条不是用户说的**（U70）——内核自己投进会话的一句话（当前只有一件：后台命令跑完了）。
   *
   * 为什么要有这一位：那种条目**走的是同一条 `user` 通道**（它要说给模型听，而模型认得
   * 的正是那条通道），但**说话的人不是用户**。没有这一位的话，两处会当场说错：
   * - **会话标题**——首条消息摘要会把「（系统通知）后台命令…」当作用户开场白；
   * - **屏上那一行**——重建会话时它长得和用户自己打的那句一模一样（青底 ＋ `› `），
   *   等于替用户捏了一句他没说过的话。
   *
   * 故这一位是**说话人**的标记，不是内容分类：带它的条目照旧进上下文（模型照读），
   * 只是不再冒充用户。
   */
